...
def merge_files(folder):
    ...
    #output results
    with open(os.path.join(folder,'Results_advanced.csv'), 'wb') as output:
        writer = csv.writer(output, quoting=csv.QUOTE_NONNUMERIC)
        for row in results_header:
            writer.writerow(row)
    
    print("\nAll files successfully merged. Output written to Results_advanced.csv file.\n")
...
if __name__ == '__main__':
    ...
    num_files = request_campaign_data_advanced(results, access_token)
